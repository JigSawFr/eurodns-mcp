...
npm run build
npm test
npm run tdqs     # lint the tool definitions with the registries' reference linter, no model
...
standard, and names the one that falls short; `npm run tdqs` runs the registries' own linter
on top of it — see [How the descriptions are scored](tools.md#how-the-descriptions-are-scored).
