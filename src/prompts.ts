  ...
  server.registerPrompt(
    ...
    ({ withinDays }) => {
      ...
      return text(
        ...
        '',
        `1. eurodns_subscription_search covers SSL, email, premium DNS and the other products`,
        `   in one call, with the auto-renewal setting. Start there rather than with one call`,
        `   per product.`,
        `2. Where it is not enough, fall back to eurodns_domain_search and, called without an`,
        `   id, eurodns_ssl_get_subscription, eurodns_email_get_subscription and`,
