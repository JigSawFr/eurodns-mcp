  ...
  server.registerPrompt(
    ...
    ({ withinDays }) => {
      ...
      return text(
        ...
        '',
        `1. eurodns_subscription_get, called without product or id, covers SSL, email, premium`,
        `   DNS and the other products in one call, with the auto-renewal setting. Start there`,
        `   rather than with one call per product.`,
        `2. Where it is not enough, fall back to eurodns_domain_get called without a`,
        `   domainName for the domains, and to eurodns_subscription_get with product set to`,
        `   ssl, email or premium_dns for the full record of one product.`,
